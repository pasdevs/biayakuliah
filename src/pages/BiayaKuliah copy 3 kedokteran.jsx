...
export default function HalamanDetailBiaya() {
  ...
  const isKedokteran = selected?.isKedokteran === true;
  ...
  const s = useMemo(() => {
    ...
    if (isKedokteran) {
      const g = selected.gelombang?.find((x) => x.gelombang === gelombang);
      return g?.semesters?.find((x) => x.semester === semester) || g?.semesters?.[0];
    }

    return selected?.semesters?.find((x) => x.semester === semester) || selected?.semesters?.[0];
  }, [selected, semester, gelombang, isKedokteran]);
  ...
  const semesterOptions = useMemo(() => {
    ...
    if (isKedokteran) {
      ...
      list = selected?.semesters || [];
    }

    const nums = list.map((x) => x.semester).filter(Boolean);
    const uniq = Array.from(new Set(nums)).sort((a, b) => a - b);
    return uniq.length ? uniq : [1, 2, 3, 4, 5, 6, 7];
  }, [selected, gelombang, isKedokteran]);
      ...
      <main className="flex-1">
        <div className="mx-auto max-w-6xl px-6 py-10">
          ...
          <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            {isKedokteran && (
              <div className="rounded-3xl border border-neutral-200 bg-white p-4 shadow-sm">
                ...
                <select
                  value={gelombang}
                  onChange={(e) => setGelombang(Number(e.target.value))}
                  className="mt-2 w-full rounded-2xl border border-neutral-200 bg-white px-4 py-2.5 text-sm"
                >
                  {selected?.gelombang?.map((g) => (
                    <option key={g.gelombang} value={g.gelombang}>
          ...
          <section className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
            {!isKedokteran ? (
              <>
                <StatCard tone="green" label={`${labelCicilan1} (Cicilan 1)`} value={formatIDR(s?.cicilan1 || 0)} />
                <StatCard tone="amber" label="Pelunasan Semester (Cicilan 2)" value={formatIDR(s?.cicilan2 || 0)} />
                <StatCard tone="slate" label="Total Biaya Semester" value={formatIDR((s?.cicilan1 || 0) + (s?.cicilan2 || 0))} />
              </>
            ) : (
              <>
                <StatCard tone="green" label="Tahap 1" value={formatIDR(s?.tahap1 || 0)} />
                <StatCard tone="amber" label="Tahap 2" value={formatIDR(s?.tahap2 || 0)} />
                <StatCard tone="slate" label="Tahap 3" value={formatIDR(s?.tahap3 || 0)} />
              </>
            )}
          </section>
